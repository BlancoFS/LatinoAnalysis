...
class ShapeFactory:
    ...
    def getvariable(self, tag, mass, cat):
        ...
        if tag in self._variables:
            try:
                ...
                self._logger.error(
                    'Variable '+tag+' not available. Possible values: '+', '.join(iter(self._variables.keys())))
                raise ke
        else:
            theVariable = tag
    ...
    def makeNominals(self, inputDir, outputDir, variables, cuts, samples, nuisances, supercut, number=99999):
        ...
        if number != 99999:
            self._outputFileName = outputDir+'/plots_' + \
                ...
                if not (len(sample['isData']) == 1 and sample['isData'][0] == 'all'):
                    ...
                    for numisDataList in range(0, len(sample['isData'])):
                        if sample['isData'][numisDataList] == '0':
                            sample['weights'][numisDataList] = "( (" + sample['weights'][numisDataList] + ") * " + str(
                                self._lumi) + ")"
                            print(" sample ['weights'][", numisDataList,
                                  "] = ", sample['weights'][numisDataList])
        ...
        #                                                                 skipMissingFiles
        inputs, evlists = self._connectInputs(
            list_of_trees_to_connect, inputDir, False)
            ...
            if 'kind' in nuisance:
                if nuisance['kind'] == 'tree':
                    ...
                    for sampleNuisName, configurationNuis in nuisance['samples'].items():
                        for sampleName, sample in self._samples.items():
                            if sampleName == sampleNuisName:
                                list_of_trees_to_connect[sampleNuisName] = [os.path.basename(
                                    s) if '###' in s else s for s in self._samples[sampleNuisName]['name']]
                    ...
                    if unskimmedFriendsDir == None:
                        #                                                                                                        skipMissingFiles
                        inputsNuisanceUp[nuisanceName], evlistsNuisanceUp[nuisanceName] = self._connectInputs(
                            list_of_trees_to_connect, nuisance['folderUp'], True)
                        inputsNuisanceDown[nuisanceName], evlistsNuisanceDown[nuisanceName] = self._connectInputs(
                            list_of_trees_to_connect, nuisance['folderDown'], True)
                    else:
                        inputsNuisanceUp[nuisanceName], evlistsNuisanceUp[nuisanceName] = self._connectInputs(
                            list_of_trees_to_connect, nuisance['unskimmedFolderUp'], True, unskimmedFriendsDir, skimListFolderUp)
                        inputsNuisanceDown[nuisanceName], evlistsNuisanceDown[nuisanceName] = self._connectInputs(
                            list_of_trees_to_connect, nuisance['unskimmedFolderDown'], True, unskimmedFriendsDir, skimListFolderDown)
        ...
        for cutName, cut in self._cuts.items():
            ...
            for sampleName, sample in self._samples.items():
                if 'weights' in list(sample.keys()):
                    self._filterTrees(sample['weight'], sample['weights'], '(' + cut +
                                      ') && (' + supercut + ')', inputs[sampleName], cutName, sampleName)
                else:
                    self._filterTrees(sample['weight'], [
                    ], '(' + cut + ') && (' + supercut + ')', inputs[sampleName], cutName, sampleName)
                            ...
                            for sampleName, sample in self._samples.items():
                                for sampleNuisName, configurationNuis in nuisance['samples'].items():
                                    if sampleNuisName == sampleName:  # check if it is the sample I'm analyzing!
                                        ...
                                        newSampleNameUp = sampleName + \
                                            '_' + nuisance['name'] + 'Up'
                                        newSampleNameDo = sampleName + \
                                        ...
                                        newSampleWeightUp = sample['weight'] + \
                                            '* (' + configurationNuis[0] + ')'
                                        newSampleWeightDo = sample['weight'] + \
                                            '* (' + configurationNuis[1] + ')'
                                        ...
                                        if 'weights' in list(sample.keys()):
                                            self._filterTrees(sample['weight'], sample['weights'], '(' + cut + ') && (' + supercut + ')',
                                                              inputsNuisanceUp[nuisanceName][sampleName], cutName, newSampleNameUp, evlistsNuisanceUp[nuisanceName][sampleName])
                                        else:
                                            self._filterTrees(sample['weight'], [], '(' + cut + ') && (' + supercut + ')', inputsNuisanceUp[nuisanceName]
                                                              [sampleName], cutName, newSampleNameUp, evlistsNuisanceUp[nuisanceName][sampleName])

                                        if 'weights' in list(sample.keys()):
                                            self._filterTrees(sample['weight'], sample['weights'], '(' + cut + ') && (' + supercut + ')',
                                                              inputsNuisanceDown[nuisanceName][sampleName], cutName, newSampleNameDo, evlistsNuisanceDown[nuisanceName][sampleName])
                                        else:
                                            self._filterTrees(sample['weight'], [], '(' + cut + ') && (' + supercut + ')', inputsNuisanceDown[nuisanceName]
                                                              [sampleName], cutName, newSampleNameDo, evlistsNuisanceDown[nuisanceName][sampleName])
            ...
            for variableName, variable in self._variables.items():
                ...
                for sampleName, sample in self._samples.items():
                    ...
                    if 'weights' in list(sample.keys()):
                        outputsHisto = self._draw(variable['name'], variable['range'], sample['weight'], sample['weights'],
                                                  cut, sampleName, inputs[sampleName], doFold, cutName, variableName, sample, True)
                    else:
                        outputsHisto = self._draw(variable['name'], variable['range'], sample['weight'], [
                        ],                 cut, sampleName, inputs[sampleName], doFold, cutName, variableName, sample, True)
                                        ...
                                        if sampleNuisName == sampleName:  # check if it is the sample I'm analyzing!
                                            if configurationNuis['typeStat'] == 'uni':
                                                ...
                                                outputsHistoUp = outputsHisto.Clone(
                                                    "histo_"+sampleName+"_statUp")
                                                outputsHistoDo = outputsHisto.Clone(
                                                    "histo_"+sampleName+"_statDown")
                                                # scale up/down
                                                self._scaleHistoStat(
                                                    outputsHistoUp,  1)
                                                self._scaleHistoStat(
                                                    outputsHistoDo, -1)
                                                # save the new two histograms in final root file
                                            ...
                                            if configurationNuis['typeStat'] == 'bbb':
                                                ...
                                                if 'keepNormalization' in list(configurationNuis.keys()):
                                                    ...
                                                    print(
                                                        " keepNormalization = ", keepNormalization)

                                                # scale up/down
                                                zeroMC = False
                                                if configurationNuis['zeroMCError'] == '1':
                                                    zeroMC = True
                                ...
                                if nuisance['kind'] == 'weight':
                                    ...
                                    for sampleNuisName, configurationNuis in nuisance['samples'].items():
                                        if sampleNuisName == sampleName:  # check if it is the sample I'm analyzing!
                                            ...
                                            if 'weights' in list(sample.keys()):
                                                outputsHistoUp = self._draw(variable['name'], variable['range'], newSampleWeightUp, sample[
                                                                            'weights'], cut, newSampleNameUp, inputs[sampleName], doFold, cutName, variableName, sample, False)
                                            else:
                                                outputsHistoUp = self._draw(variable['name'], variable['range'], newSampleWeightUp, [
                                                ],                 cut, newSampleNameUp, inputs[sampleName], doFold, cutName, variableName, sample, False)

                                            if 'weights' in list(sample.keys()):
                                                outputsHistoDo = self._draw(variable['name'], variable['range'], newSampleWeightDo, sample[
                                                                            'weights'], cut, newSampleNameDo, inputs[sampleName], doFold, cutName, variableName, sample, False)
                                            else:
                                                outputsHistoDo = self._draw(variable['name'], variable['range'], newSampleWeightDo, [
                                                ],                 cut, newSampleNameDo, inputs[sampleName], doFold, cutName, variableName, sample, False)
                                    ...
                                    for sampleNuisName, configurationNuis in nuisance['samples'].items():
                                        if sampleNuisName == sampleName:  # check if it is the sample I'm analyzing!
                                            ...
                                            newSampleWeightDo = sample['weight'] + \
                                                '* (' + \
                                                configurationNuis[1] + ")"
                                            print(" newSampleWeightUp = ",
                                                  newSampleWeightUp)
                                            print(" newSampleWeightDo = ",
                                                  newSampleWeightDo)
                                            ...
                                            if 'weights' in list(sample.keys()):
                                                outputsHistoUp = self._draw(variable['name'], variable['range'], newSampleWeightUp, sample['weights'], cut,
                                                                            newSampleNameUp, inputsNuisanceUp[nuisanceName][sampleName], doFold, cutName, variableName, sample, False)
                                            else:
                                                outputsHistoUp = self._draw(variable['name'], variable['range'], newSampleWeightUp, [
                                                ],                 cut, newSampleNameUp, inputsNuisanceUp[nuisanceName][sampleName], doFold, cutName, variableName, sample, False)

                                            if 'weights' in list(sample.keys()):
                                                outputsHistoDo = self._draw(variable['name'], variable['range'], newSampleWeightDo, sample['weights'], cut,
                                                                            newSampleNameDo, inputsNuisanceDown[nuisanceName][sampleName], doFold, cutName, variableName, sample, False)
                                            else:
                                                outputsHistoDo = self._draw(variable['name'], variable['range'], newSampleWeightDo, [
                                                ],                 cut, newSampleNameDo, inputsNuisanceDown[nuisanceName][sampleName], doFold, cutName, variableName, sample, False)
    ...
    def _symmetrize(self, hUp, hDo):
        ...
        if hUp.GetDimension() == 1:
            ...
            print(
                'No way! I am not going to symmetrize something that is not already folded into 1D')
    ...
    def _draw(self, var, rng, global_weight, weights, cut, sampleName, inputs, doFold, cutName, variableName, sample, fixZeros):
        ...
        for tree in inputs:
            print('        {0:<20} : {1:^9}'.format(
                sampleName, tree.GetEntries()), end=' ')
            # new histogram
...
#                 b1d = ((i-1)+(j-1)*nx)+1
    ...
    def _scaleHistoStatBBB(self, histo, direction, iBinToChange, keepNormalization, zeroMC=False):
        ...
        if zeroMC == False:
            ...
            for iBin in range(1, histo.GetNbinsX()+1):
                ...
                if iBin == iBinToChange:
                    if value == 0:
                        print("###DEBUG: 0 MC stat --> value = ",
                              value, " error = ", error)
                        if direction == 1:
                            print("###DEBUG: lumi = ", float(
                                self._lumi), " basew = ", basew)
    ...
    def _fixNegativeBin(self, histoNew, histoReference):
        ...
        for ibin in range(1, histoNew.GetNbinsX()+1):
            ...
            if histoNew.GetBinContent(ibin) * histoReference.GetBinContent(ibin) < 0:
                # do not put 0 to avoid bogus pogus ...
                histoNew.SetBinContent(
                    ibin, histoReference.GetBinContent(ibin) * 0.0001)
    ...
    def _fixNegativeBinAndError(self, histogram_to_be_fixed):
        ...
        for ibin in range(1, histogram_to_be_fixed.GetNbinsX()+1):
            if histogram_to_be_fixed.GetBinContent(ibin) - histogram_to_be_fixed.GetBinErrorLow(ibin) < 0:
                histogram_to_be_fixed.SetBinError(
                    ibin, histogram_to_be_fixed.GetBinContent(ibin))
    ...
    def _getBaseW(self, histo):
        ...
        baseW = histo.Integral()/histo.GetEntries() / \
            self._lumi if histo.GetEntries() > 0 else 0
    ...
    def _moveAddBin(h, fromBin, toBin):
        ...
        if dims.count(dims[0]) != len(dims):
            raise ValueError(
                'histogram and the 2 bins don\'t have the same dimension')
    ...
    def _connectInputs(self, samples, inputDir, skipMissingFiles, friendsDir=None, skimListDir=None):
        ...
        for process, filenames in samples.items():
            ...
            lists[process] = []
            trees = self._buildchain(treeName, [(
                inputDir + '/' + f) if '###' not in f else f.replace("#", "") for f in filenames],      skipMissingFiles)
            # if we specify a friends tree directory we need to load the friend treaes and attch them
            if friendsDir != None:
                friendTrees = self._buildchain(treeName, [(
                    friendsDir + '/' + f) if '###' not in f else f.replace("#", "") for f in filenames], False)
                ifriend = 0
                for friendTree in friendTrees:
                    ...
                    if friendTree.GetEntries() != trees[ifriend].GetEntries():
                        raise RuntimeError('tree '+trees[ifriend].GetFile().GetName()+" and friend tree "+friendTree.GetFile(
                        ).GetName()+" have different number of entries. you may have messed up your configuration")

                    trees[ifriend].AddFriend(friendTree)
            # if we specify a directory with skim event lists we need to load them and skim
            if skimListDir != None:
                eventlists = self._geteventlists("prunerlist",  [(
                    skimListDir + '/' + f) if '###' not in f else f.replace("#", "") for f in filenames])
    ...
    def _testEosFile(self, path):
        ...
        if 'eosuser.cern.ch' in path:
            if os.system(eoususer+' ls '+path.split('/eosuser.cern.ch/')[1]) == 0:
                return True
        if 'eoscms.cern.ch' in path:
            if os.system('eos ls '+path.split('/eoscms.cern.ch/')[1]) == 0:
                return True
        return False

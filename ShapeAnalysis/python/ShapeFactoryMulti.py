...
class ShapeFactory:
    ...
    def makeNominals(self, inputDir, outputDir, variables, cuts, samples, nuisances, supercut, number=99999, firstEvent=0, nevents=-1):
        ...
        if number != 99999:
            outputFileName = outputDir+'/plots_' + \
                self._tag+"_"+str(number)+".root"
        else:
        ...
        for sampleName, sample in self._samples.items():
                ...
                for iw, weight in enumerate(sample['weights']):
                    ...
                    if not isData[iw]:
                        if lumiscale is None:
                            lumiscale = ROOT.multidraw.ReweightSource(
                                str(self._lumi))

                        treeweight = ROOT.multidraw.ReweightSource(
                            treeweight, lumiscale)
            ...
            if len(treeweights) != len(sample['name']):
                raise RuntimeError(
                    'Number of tree-by-tree weights doesn\'t match the number of trees')

            if type(sample['weight']) is list:
                # compound weight
                sampleweight = ShapeFactory._make_reweight(sample['weight'][0])
                for w in sample['weight'][1:]:
                    sampleweight = ROOT.multidraw.ReweightSource(
                        sampleweight, ShapeFactory._make_reweight(w))
            else:
                sampleweight = ShapeFactory._make_reweight(sample['weight'])

            # Create the nominal drawer

            drawer = self._connectInputs(
                sampleName, sample['name'], inputDir, skipMissingFiles=False)
            ...
            basenames = [os.path.basename(
                s) if '###' in s else s for s in sample['name']]
            ...
            for nuisanceName, nuisance in nuisances.items():
                ...
                twosided = (
                    'OneSided' not in nuisance or not nuisance['OneSided'])
                ...
                if nkind == 'tree' or nkind == 'suffix' or nkind == 'branch_custom':  # jhchoi
                    ...
                    variations = ['V%dVar' % ivar for ivar in range(
                        len(nuisance['samples'][sampleName]))]

                skipMissing = (
                    'synchronized' in nuisance and not nuisance['synchronized'])
                if 'nominalAsAlt' in nuisance and nuisance['nominalAsAlt']:
                    ...
                    if sample['name'][0].startswith('###'):
                        altDir = os.path.dirname(
                            sample['name'][0].replace('###', ''))
                    else:
                        altDir = inputDir
                else:
                    altDir = ''
                ...
                if nkind.startswith('tree'):
                    ...
                    for var in variations:
                        if 'folder' + var in nuisance:
                            if 'unskimmedFriendTreeDir' in list(nuisance.keys()):
                                ...
                                ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(sampleName, basenames, nuisance['unskimmedFolder' + var], skipMissingFiles=skipMissing, friendsDir=(
                                    unskimmedFriendsDir, nuisanceName + var), skimListDir=skimListFolderVar, altDir=altDir)

                            else:
                                ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(
                                    sampleName, basenames, nuisance['folder' + var], skipMissingFiles=skipMissing, altDir=altDir)
                            ...
                            ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(
                                sampleName, nuisance['files' + var][sampleName], '', skipMissingFiles=False)
                    ...
                    for var in variations:
                        if 'folder' + var in nuisance:
                            ...
                            ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(
                                sampleName, sample['name'], inputDir, skipMissingFiles=False, friendsDir=(nuisance['folder' + var], friendAlias))
                            prefix = friendAlias + '.'
                        else:
                            ndrawer = nuisanceDrawers[nuisanceName][var] = self._connectInputs(
                                sampleName, sample['name'], inputDir, skipMissingFiles=False)
                        ...
                        for bname in bmap['branches']:
                            ...
                            ndrawer.replaceBranch(
                                bname, prefix + bname + bmap['suffix'])

                        ndrawers.append(ndrawer)
                # [jhchoi]pick up sysbranch and sysfiles to replace given nominal
                elif nkind.startswith('branch_custom'):
                    print("--branch_custom--")
                    for var in variations:
                        ...
                        # nuisance['BrFromToUp]  : a dictionary whose key =From , value ; To
                        for From, To in list(nuisance['BrFromTo'+var].items()):
            ...
            for aliasName, alias in self.aliases.items():
                ...
                if 'class' in alias:
                    drawer.addAlias(
                        aliasName, ShapeFactory._make_ttreefunction(alias))
                else:
            ...
            for nuisanceName, ndrawers in nuisanceDrawers.items():
                ...
                for ivar, (var, ndrawer) in enumerate(ndrawers.items()):
                    if float(sampleVarWeights[ivar]) != 1.:
                        nuisanceShift = ShapeFactory._make_reweight(
                            sampleVarWeights[ivar])
                        nuisanceweight = ROOT.multidraw.ReweightSource(
                            sampleweight, nuisanceShift)
                    else:
                    ...
                    for it, w in enumerate(treeweights):
                        if w is not None:
                            ...
                            if warnIfTreeWeight:
                                print('Nuisance', nuisanceName, 'tree filler for sample', sampleName,
                                      'has different number of trees from the nominal filler. Tree-based reweighting may cause problems.')
                                warnIfTreeWeight = False
            ...
            for cutKey, cut in cuts.items():
                ...
                if 'categories' in cut:
                    if type(cut['categories']) is dict:
                        ...
                        drawer.setCategorization(
                            cutFullName, cut['categorization'])
                    ...
                    if nuisanceName in nuisanceDrawers:
                        for ndrawer in nuisanceDrawers[nuisanceName].values():
                            ...
                            if 'categorization' in cut:
                                ndrawer.setCategorization(
                                    cutFullName, cut['categorization'])
                            else:
                                for catname in categoryOrdering:
                                    ndrawer.addCategory(
                                        cutFullName, cut['categories'][catname])
                ...
                for variableName, variable in self._variables.items():
                    ...
                    if 'weight' in variable:
                        reweight = ShapeFactory._make_reweight(
                            variable['weight'])
                    else:
                        reweight = None
                    ...
                    if 'tree' in variable:  # variable is actually a tree definition
                        def setup_filler(drawer, reweight, variation=''):
                            ...
                            treeName = 'tree_' + \
                                outputFormat.format(
                                    sample=sampleName, subsample=slabel, nuisance=nlabel)

                            if 'categories' in cut:
                                treelist = ROOT.TObjArray()

                                for catname in categoryOrdering:
                                    outFile.cd(cutName + '_' +
                                               catname + '/' + variableName)

                                    tree = ROOT.TTree(treeName, treeName)
                                    _allplots.add(tree)
                                    treelist.Add(tree)

                                filler = drawer.addTreeList(
                                    treelist, cutFullName)
                    ...
                    # variable is a real variable defined by an expression ('name') or an object ('class')
                    else:
                        if 'name' in variable:
                            if type(variable['name']) is str:
                                try:
                                    xexpr, yexpr = ShapeFactory._splitexpr(
                                        variable['name'])
                        ...
                        def setup_filler(drawer, reweight, variation=''):
                            ...
                            if 'categories' in cut:
                                ...
                                for catname in categoryOrdering:
                                    ...
                                    hTotal = self._makeshape(
                                        histoName, variable['range'])
                                ...
                                if yexpr:
                                    filler = drawer.addPlotList2D(
                                        histlist, xexpr, yexpr, cutFullName)
                                else:
                                    filler = drawer.addPlotList(
                                        histlist, xexpr, cutFullName)
                                ...
                                if yexpr:
                                    filler = drawer.addPlot2D(
                                        hTotal, xexpr, yexpr, cutFullName)
                                else:
                                    filler = drawer.addPlot(
                                        hTotal, xexpr, cutFullName)
                        ...
                        if nuisanceName in nuisanceDrawers:
                            ...
                            for ivar, var in enumerate(variations):
                                if 'tree' in variable:
                                    ...
                                    nominal_filler.addBranch(
                                        'reweight_' + nuisance['name'] + var, sampleVarWeights[ivar])
                                else:
                                    reweightNuis = ROOT.multidraw.ReweightSource(
                                        sampleVarWeights[ivar])
                                    if reweight is not None:
                                        # compound reweight
                                        reweightNuis = ROOT.multidraw.ReweightSource(
                                            reweight, reweightNuis)

                                    setup_filler(
                                        drawer, reweightNuis, nuisance['name'] + var)
            ...
            for cutKey, cut in cuts.items():
                ...
                histoName = 'histo_' + \
                    outputFormat.format(sample=sampleName,
                                        subsample=slabel, nuisance='')
                ...
                for variableName, variable in self._variables.items():
                    ...
                    if 'categories' in cut:
                        ...
                        catsuffixes = [
                            '_' + catname for catname in cut['categories']]
                    else:
                        catsuffixes = ['']
                    ...
                    for catsuffix in catsuffixes:
                        ...
                        # fold if needed
                        if 'fold' in variable:
                            print('   ', cutName + catsuffix + '/' +
                                  variableName + '/' + histoName)
                        ...
                        outputsHisto = self._postplot(
                            hTotal, doFold, cutName, sample, True, unroll2d=unroll2d, FixNegativeAfterHadd=self.FixNegativeAfterHadd)
                        ...
                        for nuisanceName, nuisance in nuisances.items():
                            ...
                            if nuisanceName == 'stat':
                                ...
                                if configurationNuis['typeStat'] == 'uni':
                                    ...
                                    upName = 'histo_' + \
                                        outputFormat.format(
                                            sample=sampleName, subsample=slabel, nuisance='_statUp')
                                    downName = 'histo_' + \
                                        outputFormat.format(
                                            sample=sampleName, subsample=slabel, nuisance='_statDown')
                                    outputsHistoUp = outputsHisto.Clone(upName)
                                    outputsHistoDo = outputsHisto.Clone(
                                        downName)
                                ...
                                # bin-by-bin
                                    ...
                                    # print "     >> bin-by-bin"
                                    keepNormalization = 'keepNormalization' in configurationNuis and int(
                                        configurationNuis['keepNormalization'])
                                    print(" keepNormalization = ",
                                          keepNormalization)
                                    zeroMC = 'zeroMCError' in configurationNuis and int(
                                        configurationNuis['zeroMCError'])

                                    for iBin in range(1, rnp.hist2array(outputsHisto, copy=False).size + 1):
                                        # take histogram --> outputsHisto
                                        upName = 'histo_' + \
                                            outputFormat.format(
                                                sample=sampleName, subsample=slabel, nuisance='_ibin_%d_statUp' % iBin)
                                        downName = 'histo_' + \
                                            outputFormat.format(
                                                sample=sampleName, subsample=slabel, nuisance='_ibin_%d_statDown' % iBin)
                                        outputsHistoUp = outputsHisto.Clone(
                                            upName)
                                        outputsHistoDo = outputsHisto.Clone(
                                            downName)
                                        _allplots.add(outputsHistoUp)
                                        ...
                                        self._scaleHistoStatBBB(
                                            outputsHistoUp,  1, iBin, keepNormalization, zeroMC)
                                        self._scaleHistoStatBBB(
                                            outputsHistoDo, -1, iBin, keepNormalization, zeroMC)

                                        # fix negative bins not consistent
                                        self._fixNegativeBin(
                                            outputsHistoUp, outputsHisto)
                                        self._fixNegativeBin(
                                            outputsHistoDo, outputsHisto)
                            ...
                            if nuisance['kind'].endswith('_envelope') or nuisance['kind'].endswith('_rms'):
                                for ivar in range(len(configurationNuis)):
                                    histoNameVar = 'histo_' + outputFormat.format(
                                        sample=sampleName, subsample=slabel, nuisance=('_%sV%dVar' % (nuisance['name'], ivar)))
                                    hTotalVar = outDir.Get(histoNameVar)
                                    _allplots.remove(hTotalVar)
                                    outputsHistoVar = self._postplot(
                                        hTotalVar, doFold, cutName, sample, False, unroll2d=unroll2d, FixNegativeAfterHadd=self.FixNegativeAfterHadd)
                                    _allplots.add(outputsHistoVar)

                                continue

                            histoNameUp = 'histo_' + outputFormat.format(
                                sample=sampleName, subsample=slabel, nuisance=('_%sUp' % nuisance['name']))
                            histoNameDown = 'histo_' + outputFormat.format(
                                sample=sampleName, subsample=slabel, nuisance=('_%sDown' % nuisance['name']))

                            hTotalUp = outDir.Get(histoNameUp)
                            _allplots.remove(hTotalUp)
                            outputsHistoUp = self._postplot(
                                hTotalUp, doFold, cutName, sample, False, unroll2d=unroll2d, FixNegativeAfterHadd=self.FixNegativeAfterHadd)
                            _allplots.add(outputsHistoUp)

                            if twosided:
                                hTotalDown = outDir.Get(histoNameDown)
                                _allplots.remove(hTotalDown)
                                outputsHistoDo = self._postplot(
                                    hTotalDown, doFold, cutName, sample, False, unroll2d=unroll2d, FixNegativeAfterHadd=self.FixNegativeAfterHadd)
                            else:
                                outputsHistoDo = outputsHisto.Clone(
                                    histoNameDown)
    ...
    def _filterTrees(self, global_weight, weights, cut, inputs, cutName, sampleName, evlists=[]):
        ...
        for itree, tree in enumerate(inputs):
            ...
            # get the list
            if len(evlists):
                print("applying the following event list to the input tree",
                      tree.GetFile().GetName())
                evlists[itree].Print()
                tree.SetEventList(evlists[itree])
            myList = ROOT.TEventList(
                'myList'+'_'+str(numTree)+'_'+sampleName+'_'+cutName, "")
            ...
            tree.Draw('>> myList'+'_'+str(numTree)+'_' +
                      sampleName+'_'+cutName, globalCut)
            # myList.Print("all")
    ...
    def _postplot(hTotal, doFold, cutName, sample, fixZeros, unroll2d=True, FixNegativeAfterHadd=False):
        ...
        if unroll2d and hTotal.InheritsFrom(ROOT.TH2.Class()):
            ...
            hTotal = ROOT.TH1D(name, hTotal.GetTitle(),
                               nx * ny, 0., float(nx * ny))
            ...
            sumw22d = rnp.array(hTotal2d.GetSumw2(),
                                copy=False).reshape((ny + 2, nx + 2))
    ...
    def _scaleHistoStatBBB(self, histo, direction, iBinToChange, keepNormalization, zeroMC=False):
        ...
        if cont.flat[iBinToChange - 1] == 0. and zeroMC and direction == 1:
            ...
            cont_uo = rnp.hist2array(
                histo, copy=False, include_overflow=True).T
            sumw2 = rnp.array(histo.GetSumw2(),
                              copy=False).reshape(cont_uo.shape)
            ...
            if len(sumw2.shape) == 2:
                ...
                sumw2 = sumw2[1:-1]
            cont.flat[iBinToChange - 1] += direction * \
                np.sqrt(sumw2.flat[iBinToChange - 1])
    ...
    def _fixNegativeBinAndError(histogram_to_be_fixed):
        ...
        cont = rnp.hist2array(histogram_to_be_fixed,
                              copy=False, include_overflow=True).T.flat
    ...
    def _connectInputs(self, process, filenames, inputDir, skipMissingFiles, friendsDir=None, skimListDir=None, altDir=''):
        ...
        if friendsDir is not None:
            ...
            self._buildchain(drawer, files, False, friendtree=(
                self._treeName, friendsDir[1]))
    ...
    def _testEosFile(self, path):
        ...
        if 'eosuser.cern.ch' in path:
            if os.system(eoususer+' ls '+path.split('/eosuser.cern.ch/')[1]+' >/dev/null 2>&1') == 0:
                return True
        if 'eoscms.cern.ch' in path:
            if os.system('eos ls '+path.split('/eoscms.cern.ch/')[1]+' >/dev/null 2>&1') == 0:
                return True
        return False
    ...
    def _test_sdfarm_File(self, path):
        if 'cms-xrdr.sdfarm.kr' in path:
            if 'xrd//store' in path:
                cmd = 'xrd cms-xrdr.sdfarm.kr existfile /xrd/store/' + \
                    path.split('xrd//store/')[1]
    ...
    def _buildchain(self, drawer, files, skipMissingFiles, friendtree=None, altDir=''):
        ...
        for path in files:
            for att in range(5):  # try opening the file 5 times
                ...
                print(
                    'File '+path+' doesn\'t exist and skipMissingFiles=False in buildChain.')
            ...
            for catsuffix in catsuffixes:
                for variableName, variable in variables.items():
                    ...
                    for sampleName, sample in samples.items():
                        ...
                        if 'subsamples' in sample:
                            slabels = list(
                                '_%s' % ss for ss in sample['subsamples'].keys())
                        else:
                            slabels = ['']
                        ...
                        for slabel in slabels:
                            histoName = 'histo_' + \
                                outputFormat.format(
                                    sample=sampleName, subsample=slabel, nuisance='')
                            nominal = outDir.Get(histoName)
                            vnominal = rnp.hist2array(nominal, copy=False)

                            variations = np.empty(
                                (len(configurationNuis), vnominal.size), dtype=vnominal.dtype)

                            for ivar in range(len(configurationNuis)):
                                histoNameVar = 'histo_' + outputFormat.format(
                                    sample=sampleName, subsample=slabel, nuisance=('_%sV%dVar' % (nuisance['name'], ivar)))
                                hTotalVar = outDir.Get(histoNameVar)
                                variations[ivar, :] = rnp.hist2array(
                                    hTotalVar, copy=True).flat
                            ...
                            if nuisance['kind'].endswith('_envelope'):
                                ...
                                arrnom = np.tile(
                                    vnominal.flat, (variations.shape[0], 1))
                                arrv = np.sqrt(
                            ...
                            if 'suppressNegativeNuisances' in sample and (cutName in sample['suppressNegativeNuisances'] or 'all' in sample['suppressNegativeNuisances']):
                                ShapeFactory._fixNegativeBin(
                                    outputsHistoUp, nominal)
    ...
    def postprocess_NegativeBinAndError(nuisances, sampleName, sample, cuts, variables, outFile):
        ...
        if 'suppressNegative' in sample:
            for cutName, cut in cuts.items():
                if not (cutName in sample['suppressNegative'] or 'all' in sample['suppressNegative']):
                    continue
                ...
                for catsuffix in catsuffixes:
                    for variableName, variable in variables.items():
                        ...
                        for slabel in slabels:
                            ...
                            if ShapeFactory._fixNegativeBinAndError(nominal):
                                nominal.Write()
        ...
        if 'suppressNegativeNuisances' in sample:
            for nuisance in nuisances.values():
                ...
                if 'type' not in nuisance:
                    continue
                if not nuisance['type'].startswith('shape'):
                    continue
                for cutName, cut in cuts.items():
                    if not (cutName in sample['suppressNegativeNuisances'] or 'all' in sample['suppressNegativeNuisances']):
                        continue
                    ...
                    for catsuffix in catsuffixes:
                        for variableName, variable in variables.items():
                            ...
                            for slabel in slabels:
                                ...
                                if ShapeFactory._fixNegativeBin(outputsHistoUp, nominal):
                                    outputsHistoUp.Write()
                                if twosided:
                                    if ShapeFactory._fixNegativeBin(outputsHistoDown, nominal):
                                        outputsHistoDown.Write()

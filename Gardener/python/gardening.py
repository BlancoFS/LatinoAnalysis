...
class TreeCloner(object):
    ...
    def addMainOptions(self, parser):
        ...
        group.add_option('--auxiliaryFile', dest='auxiliaryFile',
                         help='auxiliary friend tree', default=None)
        group.add_option('--saveOnlyModifiedBranches', dest='saveOnlyModifiedBranches',
                         help='whether to save only the modified branches of the whole tree (default=false)', default=False, action="store_true")
        group.add_option('--eventListForFriend', dest='eventListForFriend',
                         help='whether to load an event list from the pruner when reading the friend tree', default=False, action="store_true")
    ...
    def connect(self, tree, input, histos2Create=[]):
        ...
        self.itreeTotalEventsTriggers = self._getRootObj(
            self.ifile, "totalEventsTriggers")
        self.itreeMcWeightExplainedOrdered = self._getRootObj(
            self.ifile, "mcWeightExplainedOrdered")
        ...
        if self.auxiliaryFile != None:
            ...
            self.friendTree = self._getRootObj(
                self.friendFile, self.itree.GetName())
    ...
    def clone(self, output, branches=[]):
        ...
        if not self.saveOnlyModifiedBranches:
            ...
            self.otree = ROOT.TTree(
                self.itree.GetName(), self.itree.GetTitle())
...
class Pruner(TreeCloner):
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-f', '--filter',   dest='filter',
                         help='cut string as undestood by TTree::Draw', default='')
        group.add_option('-d', '--drop',     dest='drops',
                         help='drops the variables while cloning', action='append', default=[])
        group.add_option('--droplist',      dest='droplist',
                         help='drops the variables listed in a config file', action='append', default=[])
        group.add_option('-k', '--keep',     dest='keeps',
                         help='keeps the variables while cloning', action='append', default=[])
        group.add_option('--keeplist',      dest='keeplist',
                         help='keeps the variables listed in a config file', action='append', default=[])
        group.add_option('--eventListOutput',   dest='eventListOutput',
                         help='whether the selected event list is to be written to file', default=False, action='store_true')
        group.add_option('-n', '--dryrun',   dest='dryrun',
                         help='do nothing, just count', action='store_true')
        parser.add_option_group(group)
        return group
        ...
        if self.keeps or self.keeplist:
            ...
            for q in range(keeps_string.size()):
                ...
                if checkBranchExists == 0:
                    print(('I cannot find branch ' + str(keeps_string.at(q)
                                                         ) + '. Be aware that you may create an empty tree!'))
...
class Grafter(TreeCloner):
    ...
    def addOptions(self, parser):
        description = self.help()
        group = optparse.OptionGroup(parser, self.label, description)
        group.add_option('-v', '--var', dest='variables',
    ...
    def process(self, **kwargs):
        ...
        for bname in self.variables:
            ...
            if (branch.GetTitle() != newtitle):
                print(('WARNING: Branch mutation detected: from',
                      branch.GetTitle(), 'to', newtitle))
                hasMutation = True

        if hasMutation:
            confirm(
                'Mutation detected. Do you _really_ want to continue?') or sys.exit(0)
...
#     @staticmethod
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-p', '--path',      dest='path',
                         help='Histogram file path',)
        group.add_option('-H', '--Histogram', dest='hist',
                         help='Histogram name',)
        group.add_option('-k', '--key',       dest='key',
                         help='key variable used to extract the weight from the histogram',)
        group.add_option('-b', '--branch',    dest='branch',
                         help='Name of the lepton efficiency weight branch')
...
def gardener_cli(modules):
    ...
    parser.add_option('-t', '--tree',        dest='tree',
                      default='latino',   help='Name of the tree to operate on (default = %default)')
    parser.add_option('-r', '--recursive',   dest='recursive',   action='store_true',
                      default=False,      help='Recurse subdirectories (default = %default)')
    parser.add_option('-F', '--force',       dest='force',       action='store_true',
                      default=False,      help='Don\'t ask for confirmation when recursing (default = %default)')
    ...
    # file1 file2 file3 > outdir
        ...
        if os.path.isdir(input):
            ...
            print(('The directory tree', input,
                  'will be gardened and copied to', output))

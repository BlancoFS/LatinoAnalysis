...
class LeptonPtCorrector(TreeCloner):
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-c', '--cmssw', dest='cmssw',
                         help='cmssw version req for met vars', default='763')
        group.add_option('--fileInScaleEle',   dest='FileWithPtScaleDataEle',
                         help='file with lep pT scale values to be applied to Data for Ele', default=None)
        group.add_option('--fileInSmearingEle', dest='FileWithPtSmearingMCEle',
                         help='file with lep pT smearing values to be applied to MC for Ele', default=None)
        group.add_option('-w', '--isData', dest='isData',
                         help='is data? 1 = data, 0 = mc', default=1)
    ...
    def checkOptions(self, opts):
        ...
        if opts.cmssw == 'Full2016':
            if opts.FileWithPtScaleDataEle == None:
                opts.FileWithPtScaleDataEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/Moriond17_23Jan_ele_scales.dat'
            print((" opts.FileWithPtScaleDataEle = ", opts.FileWithPtScaleDataEle))
            if opts.FileWithPtSmearingMCEle == None:
                opts.FileWithPtSmearingMCEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/Moriond17_23Jan_ele_smearings.dat'
            print((" opts.FileWithPtSmearingMCEle = ",
                  opts.FileWithPtSmearingMCEle))
        elif opts.cmssw == 'ICHEP2016':
            if opts.FileWithPtScaleDataEle == None:
                opts.FileWithPtScaleDataEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/80X_DCS05July_plus_Golden22_scales.dat'
            ...
            if opts.FileWithPtSmearingMCEle == None:
                opts.FileWithPtSmearingMCEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/80X_28JunPrompt_2016_smearings.dat'
            print((" opts.FileWithPtSmearingMCEle = ",
                  opts.FileWithPtSmearingMCEle))
        else:    # 2015 numbers re-reco
            if opts.FileWithPtScaleDataEle == None:
                opts.FileWithPtScaleDataEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/76X_16DecRereco_2015_scales.dat'
            print((" opts.FileWithPtScaleDataEle = ", opts.FileWithPtScaleDataEle))
            if opts.FileWithPtSmearingMCEle == None:
                opts.FileWithPtSmearingMCEle = cmssw_base + \
                    '/src/LatinoAnalysis/Gardener/python/data/lepton_corrections/76X_16DecRereco_2015_smearings.dat'
            print((" opts.FileWithPtSmearingMCEle = ",
                  opts.FileWithPtSmearingMCEle))
        ...
        self.leppTscaler = {}
        self.leppTscaler['ele'] = [line.rstrip().split()
                                   for line in file_FileWithPtScaleDataEle]
        #  e.g.        [ eta  ]    string              [     run    ]   scale   boh? uncertainties?
        #               0    1    lowR9                254790  256629  1.0028  0.0007 0.0001 0.0002

        self.leppTsmearing = {}
        self.leppTsmearing['ele'] = [line.rstrip().split(
        ) for line in file_FileWithPtSmearingMCEle if '#' not in line]
        #  e.g.
        ...
        for i in range(nentries):
            ...
            for i in range(itree.std_vector_lepton_pt.size()):
                ...
                if self.isData == 1:
                    ...
                    r9_lep = itree.std_vector_electron_R9[i]   # FIXME new
                    wt = self._getScale(
                        kindLep, pt_lep, eta_lep, itree.run, r9_lep)
                    # print " wt = ", wt
            ...
            leptonOrder = sorted(list(range(len(leptonPtChanged))),
                                 key=lambda k: leptonPtChanged[k], reverse=True)
            ...
            # update met
            self.oldBranchesToBeModifiedSimpleVariable[self.metvar1][0] = numpy.float32(
                newmet.Pt())
            self.oldBranchesToBeModifiedSimpleVariable[self.metvar2][0] = numpy.float32(
                newmet.Phi())

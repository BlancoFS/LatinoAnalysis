...
class btagPogScaleFactors(TreeCloner):
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-w', '--working-point', dest='workingPoint',
                         help='CSVv2 working point', default='0')
        group.add_option('-s', '--scalefactor-file', dest='sfFile',
                         help='file with scale factors', default='data/cMVAv2.csv')
        group.add_option('-p', '--scalefactor-file-tp', dest='sfFileTP',
                         help='file with scale factors', default='data/TPScaleFactors.csv')
        group.add_option('-e', '--efficiency-file', dest='efficiencyMCFile',
                         help='file with efficiency tables on MC', default='data/efficiencyMCFile76X.py')
    ...
    def checkOptions(self, opts):
        ...
        try:
            ROOT.gROOT.LoadMacro(
                cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/BTagCalibrationStandalone.cc+g')
        except RuntimeError:
            ROOT.gROOT.LoadMacro(
                cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/BTagCalibrationStandalone.cc++g')
        #ROOT.gROOT.ProcessLine('.L BTagCalibrationStandalone.cc+')
        print(("scale factors from", cmssw_base +
              '/src/LatinoAnalysis/Gardener/python/data/'+opts.sfFile))

        wp = int(opts.workingPoint)
        self.calib = ROOT.BTagCalibration(
            "cMVAv2", cmssw_base+'/src/LatinoAnalysis/Gardener/python/'+opts.sfFile)
        self.readerCentral = ROOT.BTagCalibrationReader(
            self.calib, wp, "ttbar", "central")
        self.readerUp = ROOT.BTagCalibrationReader(
            self.calib, wp, "ttbar", "up")
        self.readerDown = ROOT.BTagCalibrationReader(
            self.calib, wp, "ttbar", "down")
        ...
        self.readerLightCentral = ROOT.BTagCalibrationReader(
            self.calib, wp, "incl", "central")
        self.readerLightUp = ROOT.BTagCalibrationReader(
            self.calib, wp, "incl", "up")
        self.readerLightDown = ROOT.BTagCalibrationReader(
            self.calib, wp, "incl", "down")

        # our T&P scale factors
        self.calibTP = ROOT.BTagCalibration(
            "csvv2", cmssw_base+'/src/LatinoAnalysis/Gardener/python/'+opts.sfFileTP)
        self.readerCentralTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "tp", "central")
        self.readerUpTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "tp", "up")
        self.readerDownTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "tp", "down")

        self.readerLightCentralTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "zcontrol", "central")
        self.readerLightUpTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "zcontrol", "up")
        self.readerLightDownTP = ROOT.BTagCalibrationReader(
            self.calibTP, wp, "zcontrol", "down")
            ...
            for iJet in range(len(itree.std_vector_jet_pt)):
                ...
                if pt > self.minpt and abs(eta) < self.maxeta:
                    ...
                    if idJet != 2:
                        ...
                        if pt < 30:
                            sfUp = 2 * \
                                (self.readerUp.evaluate(idJet, eta, pt) - sf) + sf
                            sfDown = 2 * \
                                (self.readerDown.evaluate(idJet, eta, pt) - sf) + sf
                        else:
                        ...
                        sfTP = self.readerLightCentralTP.evaluate(
                            idJet, eta, pt)
                        sfTPUp = self.readerLightUpTP.evaluate(idJet, eta, pt)
                        sfTPDown = self.readerLightDownTP.evaluate(
                            idJet, eta, pt)
                    ...
                    pDataUp = pDataUp*effMC * \
                        sfUp if tagged else pDataUp*(1.-effMC*sfUp)
                    pDataDown = pDataDown*effMC * \
                        sfDown if tagged else pDataDown*(1.-effMC*sfDown)

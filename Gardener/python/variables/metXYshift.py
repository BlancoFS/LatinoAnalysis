...
class MetXYshiftTreeMaker(TreeCloner):
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-c', '--cmssw',  dest='cmssw',
                         help='cmssw version (naming convention may change)', default='763')
        group.add_option('-p', '--paraFile',  dest='paraFile',  help='correction parameter sets',
                         default='Spring16_V0_MET_MC_XYshiftMC_PfType1MetLocal.txt')
        group.add_option('-s', '--sample',  dest='sample',
                         help='sample to correct', default='Run2016BDouble')
    ...
    def checkOptions(self, opts):
        ...
        print(("  paraFile ="+self.cmssw_base +
              "/src/LatinoAnalysis/Gardener/python/data/met/"+self.paraFile))
    ...
    def process(self, **kwargs):
        ...
        try:
            ROOT.gROOT.LoadMacro(
                self.cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/metXYshift.C+g')
        except RuntimeError:
            ROOT.gROOT.LoadMacro(
                self.cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/metXYshift.C++g')

        MetXYshift = ROOT.metXYshift(
            self.cmssw_base+"/src/LatinoAnalysis/Gardener/python/data/met/"+self.paraFile)
        ...
        if self.cmssw == '74x':
            self.branchNamesToBeModifAdded = [
                'pfType1Met', 'pfType1Metphi', 'orgMetPfType1', 'orgMetPfType1Phi']
        else:
            self.branchNamesToBeModifAdded = [
                'metPfType1', 'metPfType1Phi', 'orgMetPfType1', 'orgMetPfType1Phi']
        ...
        if self.cmssw == '74x':
            ...
            self.otree.Branch(
                'pfType1Metphi', corrMetPfType1Phi, 'pfType1Metphi/F')
        else:
            self.otree.Branch('metPfType1',   corrMetPfType1, 'metPfType1/F')
            self.otree.Branch(
                'metPfType1Phi', corrMetPfType1Phi, 'metPfType1Phi/F')

        self.otree.Branch('orgMetPfType1',   orgMetPfType1, 'orgMetPfType1/F')
        self.otree.Branch('orgMetPfType1Phi',
                          orgMetPfType1Phi, 'orgMetPfType1Phi/F')
        ...
        varType = {"multiplicity": 0, "ngoodVertices": 1, "sumPt": 2, "pfType1": 3, "ptcMet": 4,
                   0: "multiplicity", 1: "ngoodVertices", 2: "sumPt", 3: "pfType1", 4: "ptcMet"}

...
class chargeFlipWeight(TreeCloner):
    ...
    def addOptions(self, parser):
        ...
        group.add_option('-j', '--njets',  dest='njets',
                         help='Minimum number of jets',  default='0')
    ...
    def checkOptions(self, opts):
        ...
        if self.njets == 0:
            self.chFlipEeFileName = cmssw_base + \
                '/src/LatinoAnalysis/Gardener/python/data/chFlip/DY_SSoverOS_2Dweight_inclusive.root'
            self.chFlipEeHistName = 'DY_leptoneta2D_weight'
        else:
            self.chFlipEeFileName = cmssw_base + \
                '/src/LatinoAnalysis/Gardener/python/data/chFlip/DY_SSoverOS_2Dweight_2jet_MCsubtr_Feb2017.root'
            self.chFlipEeHistName = 'data_leptonabseta2D_weight'

        self.chFlipEeFile = self._openRootFile(self.chFlipEeFileName)
        self.chFlipEeHist = self._getRootObj(
            self.chFlipEeFile, self.chFlipEeHistName)
    ...
    def _getHistoValue(self, h2, eta1, eta2):
        eta1_V = eta1
        eta2_V = eta2
        if eta1 < self.mineta_ele:
            eta1_V = -2.399
        if eta2 < self.mineta_ele:
            eta2_V = -2.399
        if eta1 > self.maxeta_ele:
            eta1_V = 2.399
        if eta2 > self.maxeta_ele:
            eta2_V = 2.399
    ...
    def process(self, **kwargs):
        ...
        for i in range(nentries):
            ...
            if pt1 > 0 and pt2 > 0 and (flav1*flav2) == (-11.*11):
                chFlipProba[0] = self._getHistoValue(
                    self.chFlipEeHist, eta1, eta2)

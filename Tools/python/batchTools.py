...
class batchJobs:
    def __init__(self, baseName, prodName, stepList, targetList, batchSplit, postFix='', usePython=False, useBatchDir=True, wDir='', JOB_DIR_SPLIT_READY=False, USE_SINGULARITY=False):
        ...
        for iStep in stepList:
            ...
            for iTarget in targetList:
                ...
                if not jName in self.jobsList:
                    self.jobsList.append(jName)
        ...
        if not os.path.exists(self.subDir):
            os.system('mkdir -p '+self.subDir)
        CMSSW = os.environ["CMSSW_BASE"]
        SCRAMARCH = os.environ["SCRAM_ARCH"]
        for jName in self.jobsList:
            if JOB_DIR_SPLIT and self.JOB_DIR_SPLIT_READY:
                subDirExtra = '/' + jName.split('__')[3]
                if not os.path.exists(self.subDir+subDirExtra):
                    os.system('mkdir -p '+self.subDir+subDirExtra)
            else:
                subDirExtra = ''
            jFile = open(self.subDir+subDirExtra+'/'+jName+'.sh', 'w')
            if usePython:
                pFile = open(self.subDir+subDirExtra+'/'+jName+'.py', 'w')
            jFile.write('#!/bin/bash\n')
            if self.USE_SINGULARITY:
                jFileSing = open(self.subDir+subDirExtra +
                                 '/'+jName+'_Sing.sh', 'w')
                if 'iihe' in hostName:
                    jFileSing.write('sl7 '+self.subDir +
            ...
            if 'cern' in hostName:
                ...
                jFile.write('export X509_USER_PROXY=/afs/cern.ch/user/' +
                            os.environ["USER"][:1]+'/'+os.environ["USER"]+'/.proxy\n')
                if 'latino' in hostName:
                    jFile.write('export X509_USER_PROXY=/eos/user/' +
                                os.environ["USER"][:1]+'/'+os.environ["USER"]+'/.proxy\n')
                    jFile.write('export EOS_MGM_URL=root://eoscms.cern.ch\n')
            elif "pi.infn.it" in socket.getfqdn():
                jFile.write('#$ -N '+jName+'\n')
                jFile.write('export X509_USER_PROXY=/home/users/' +
                            os.environ["USER"]+'/.proxy\n')
            elif 'knu' in hostName:
                jFile.write('#$ -N '+jName+'\n')
                jFile.write('export X509_USER_PROXY=/u/user/' +
                            os.environ["USER"]+'/.proxy\n')
            elif 'sdfarm' in hostName:
                jFile.write('#$ -N '+jName+'\n')
                jFile.write('export VO_CMS_SW_DIR=/cvmfs/cms.cern.ch\n')
                jFile.write('export X509_USER_PROXY=/cms/ldap_home/' +
                            os.environ["USER"]+'/.proxy\n')
            elif 'hercules' in hostName:
                jFile.write('#$ -N '+jName+'\n')
                jFile.write('export VO_CMS_SW_DIR=/cvmfs/cms.cern.ch\n')
                jFile.write('export X509_USER_PROXY=/gwpool/users/' +
                            os.environ["USER"]+'/.proxy\n')
            else:
                jFile.write('export X509_USER_PROXY=/user/' +
                            os.environ["USER"]+'/.proxy\n')
                if self.USE_SINGULARITY:
                    jFileSing.write(
                        'export X509_USER_PROXY=/user/'+os.environ["USER"]+'/.proxy\n')
            if 'CONFIGURATION_DIRECTORY' in os.environ:
                jFile.write('export CONFIGURATION_DIRECTORY=' +
                            os.environ['CONFIGURATION_DIRECTORY']+'\n')
            ...
            if useBatchDir:
                if 'iihe' in hostName:
                    ...
                    jFile.write("cd /gpfs/projects/cms/" +
                                os.environ["USER"]+"/ \n")
            ...
            if self.USE_SINGULARITY:
                ...
                jFileSing.close()
                os.system('chmod +x '+self.subDir +
                          subDirExtra+'/'+jName+'_Sing.sh')
        ...
        if 'iihe' in hostName:
            ...
            os.system('cp $X509_USER_PROXY /user/' +
                      os.environ["USER"]+'/.proxy')
        if "pi.infn.it" in socket.getfqdn():
            os.system('cp $X509_USER_PROXY /home/users/' +
                      os.environ["USER"]+'/.proxy')
        if "knu" in hostName:
            os.system('cp $X509_USER_PROXY /u/user/' +
                      os.environ["USER"]+'/.proxy')
            #os.system('cp /tmp/x509up_u$UID /u/user/'+os.environ["USER"]+'/.proxy')
        if "sdfarm" in hostName:
            os.system('cp $X509_USER_PROXY /cms/ldap_home/' +
                      os.environ["USER"]+'/.proxy')
        if "hercules" in hostName:
            os.system('cp $X509_USER_PROXY /gwpool/users/' +
                      os.environ["USER"]+'/.proxy')
    ...
    def Sub(self, queue='longlunch', IiheWallTime='168:00:00', optTodo=False):
        ...
        if 'cern' in hostName and not CERN_USE_LSF:
            flavours = ['espresso', 'microcentury', 'longlunch',
        ...
        for jName in self.jobsList:
            ...
            jFile.write('[ $? -eq 0 ] && mv '+jidFile+' ' +
                        jidFile.replace('.jid', '.done'))
            jFile.close()
            ...
            if 'cern' in hostName:
                if CERN_USE_LSF:
                    jobid = os.system(
                        'bsub -q '+queue+' -o '+outFile+' -e '+errFile+' '+jobFile+' > '+jidFile)
                else:
                    ...
                    jdsFile.write('error = '+self.subDir +
                                  subDirExtra+'/'+jName+'.err\n')
                    jdsFile.write('log = '+self.subDir +
                                  subDirExtra+'/'+jName+'.log\n')
                    if CONDOR_ACCOUNTING_GROUP:
                        jdsFile.write('+AccountingGroup = ' +
                                      CONDOR_ACCOUNTING_GROUP+'\n')
                        jdsFile.write('accounting_group = ' +
                                      CONDOR_ACCOUNTING_GROUP+'\n')
                    if AUTO_CONDOR_RETRY:
                        jdsFile.write(
                            'on_exit_hold = (ExitBySignal == True) || (ExitCode != 0)\n')
                        jdsFile.write(
                            'periodic_release =  (NumJobStarts < 3) && ((CurrentTime - EnteredCurrentStatus) > (60*3))\n')
                ...
                while nTry < 3:
                    nTry += 1
                    if self.USE_SINGULARITY:
                        print('ssh m8 qsub '+QSOPT+' -N '+jName+' -q '+queue +
                              ' -o '+outFile+' -e '+errFile+' '+jobFile+' > '+jidFile)
                        jobid = os.system('ssh m8 qsub '+QSOPT+' -N '+jName+' -q ' +
                                          queue+' -o '+outFile+' -e '+errFile+' '+jobFile+' > '+jidFile)
                    else:
                        jobid = os.system('qsub '+QSOPT+' -N '+jName+' -q '+queue +
                                          ' -o '+outFile+' -e '+errFile+' '+jobFile+' > '+jidFile)
                    print('TRY #:', nTry, '--> Jobid : ', jobid)
                    if jobid == 0:
                        nTry = 999
                    else:
                        os.system('rm '+jidFile)
                if not jobid == 0 and optTodo:
                    todoFile = open(self.subDir+subDirExtra +
                                    '/'+jName+'.todo', 'w')
                    todoFile.write('qsub '+QSOPT+' -N '+jName+' -q '+queue +
                                   ' -o '+outFile+' -e '+errFile+' '+jobFile+' > '+jidFile)
                    todoFile.close()
                ...
                jobid = os.system('qsub -q '+queue+' -o '+outFile +
                                  ' -e '+errFile+' '+jobFile+' > '+jidFile)
                ...
                print('qsub  -o '+outFile+' -e '+errFile+' ' +
                      jobFile+' -q ' + queue + ' > '+jidFile)
                jobid = os.system(
                    'qsub  -o '+outFile+' -e '+errFile+' '+jobFile+' -q ' + queue + ' > '+jidFile)
            elif 'sdfarm' in hostName:
                jdsFileName = self.subDir+subDirExtra+'/'+jName+'.jds'
                jdsFile = open(jdsFileName, 'w')
                ...
                jdsFile.write('JobBatchName = '+jName.split('__')
                              [0]+'__'+jName.split('__')[1]+'__'+jName.split('__')[2]+'\n')
                if 'ui10' in hostName:
                    jdsFile.write(
                ...
                jobid = os.system('qsub -P l.gaes -S /bin/bash -cwd -N Latino -o ' +
                                  outFile+' -e '+errFile+' '+jobFile+' -j y > '+jidFile)
            elif "pi.infn.it" in socket.getfqdn():
                queue = "cms"
                jobid = os.system('bsub -q '+queue+' -o '+outFile +
                                  ' -e '+errFile+' '+jobFile+' > '+jidFile)
            else:
                # print 'cd '+self.subDir+'/'+jName.split('/')[0]+'; bsub -q '+queue+' -o '+outFile+' -e '+errFile+' '+jName.split('/')[1]+'.sh | grep submitted'
                jobid = os.system('bsub -q '+queue+' -o '+outFile +
        ...
        if scheduler == 'condor':
            ...
            if not matches:
                ...
                proc = subprocess.Popen(['condor_q', clusterId, '-l', '-attr', 'ProcId,Cmd', '-json'],
    ...
    def AddCopy(self, iStep, iTarget, inputFile, outputFile):
        ...
        if 'iihe' in hostName:
            jFile.write(
                'lcg-cp '+inputFile+' srm://maite.iihe.ac.be:8443/pnfs/iihe/cms'+outputFile+'\n')
        elif 'ifca' in hostName:
            jFile.write('mv '+inputFile+' /gpfs/gaes/cms'+outputFile+'\n')
        elif "pi.infn.it" in socket.getfqdn():
            jFile.write(
                'lcg-cp '+inputFile+' srm://stormfe1.pi.infn.it:8444/srm/managerv2?SFN=/cms'+outputFile+'\n')
        elif 'knu' in hostName:
            jFile.write('gfal-copy '+inputFile +
                        ' srm://cluster142.knu.ac.kr:8443/srm/managerv2?SFN=/pnfs/knu.ac.kr/data/cms/'+outputFile+'\n')
        elif 'sdfarm' in hostName:
            jFile.write('gfal-copy -p '+inputFile +
                        ' srm://cms-se.sdfarm.kr:8443/srm/v2/server?SFN=/xrootd/'+outputFile+'\n')
        elif 'hercules' in hostName:
            jFile.write('gfal-copy -p file://`pwd`/' + inputFile +
                        ' srm://storm.mib.infn.it:8444/cms/' + outputFile + '\n')
        else:
...
def batchStatus():
    ...
    for iDir in DirList:
        ...
        for iFile in FileList:
            ...
            if not iStep in Done:
                Done[iStep] = 0
            if not iStep in Pend:
                Pend[iStep] = 0
            if not iStep in Runn:
                Runn[iStep] = 0
            if not iStep in Crab:
                Crab[iStep] = 0
            if not iStep in Tota:
                Tota[iStep] = 0
            if not iStep in FileRuns:
                FileRuns[iStep] = []
            Tota[iStep] += 1
            if os.path.isfile(jidFile):
                # check if not CRAB:
                iCrab = os.popen(
                    'cat '+jidFile+' | awk \'{print $1}\' ').read()
                # print iCrab
                if 'CRABTask' in iCrab:
                    Crab[iStep] += 1
                else:
                    if 'iihe' in hostName:
                        iStat = os.popen(
                            'cat '+jidFile+' | awk -F\'.\' \'{print $1}\' | xargs -n 1 qstat | grep localgrid | awk \'{print $5}\' ').read()
                        if 'Q' in iStat:
                            Pend[iStep] += 1
                        else:
                            Runn[iStep] += 1
                        ...
                        iStat = os.popen(
                            'qstat | grep \" qw \" |  awk \'{print $1 \" '+jidFile+'\"}\' | xargs -n 2 grep | awk \'{ print $2 }\' ').read()
                        if 'job' in iStat:
                            Pend[iStep] += 1
                        else:
                            Runn[iStep] += 1
                        ...
                        iStat = os.popen(
                            r'cat '+jidFile+" | xargs -n 1 condor_q | tail -n1").read()
                        if '1 idle' in iStat:
                            Pend[iStep] += 1
                        else:
                            Runn[iStep] += 1
                        ...
                        iStat = os.popen(
                            'cat '+jidFile+' | awk \'{print $2}\' | awk -F\'<\' \'{print $2}\' | awk -F\'>\' \'{print $1}\' | xargs -n 1 bjobs | grep -v "JOBID" | awk \'{print $3}\'').read()
                        if 'PEND' in iStat:
                            Pend[iStep] += 1
                        else:
                            Runn[iStep] += 1
        ...
        print('----------------------------')
        for iStep in Done:
            print('     --> '+iStep+' : PENDING= '+str(Pend[iStep])+' RUNNING= '+str(Runn[iStep])+' DONE= '+str(
                Done[iStep])+' CRAB= '+str(Crab[iStep])+' / TOTAL= '+str(Done[iStep]) + '/'+str(Tota[iStep]))
    ...
    for iDir in DirList:
        ...
        for iFile in FileList:
            ...
            if 'cern' in hostName and not CERN_USE_LSF:
                ...
                if queue not in ['shortcms', 'longcms']:
                    queue = 'shortcms'
                jobid = os.system('qsub -q '+queue+' -o '+outFile+' -e ' +
                                  errFile+' '+jobFile+' -q ' + queue + ' > '+jidFile)
                if jobid == 0:
                    os.system('rm '+iFile)
                else:
                    os.system('rm '+jidFile)

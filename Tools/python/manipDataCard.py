...
# open
        ...
        # header1, header2, block1, block2, systs
        self.advance = [False, False, False, False, False]
...
# parse
        ...
        for iline, (kline, line) in enumerate(self.lines.items()):
            fields = line.split()
            currTag = fields[0]
            if currTag == 'Observation':
                currTag = 'observation'
            if prevTag == 'Observation':
                prevTag = 'observation'
            # whereAmI
            if not kline > len(list(self.lines.keys()))-4:
                nextfour = [currTag, self.lines[kline+1].split()[0],
                            self.lines[kline+2].split()[0], self.lines[kline+3].split()[0]]
            else:
                nextfour = ['', '', '', '']
        ####################
            if set(nextfour[0:3]) == set(['imax', 'jmax', 'kmax']):
                ...
                self.advance[3] = True
                self.advance[4] = False
                # for cases where there are no shapes lines
                self.done[1] = True
        ...
        ####################
            ...
            # elif currTag == 'bin' and not prevTag == 'observation':
                ...
                if not currTag in self.content['block1']:
                    ...
                    sys.exit('Key %s already exists in block1 dictionary. \nNo algorithm to process line: %s' % (
                        currTag, line))
            # block2
            # elif currTag == 'bin' and prevTag == 'observation':
                ...
                if not currTag in self.content['block2']:
                    ...
                    sys.exit('Key %s already exists in block2 dictionary. \nNo algorithm to process line: %s' % (
                        currTag, line))
                if set(self.content['block2'].keys()) == set(['bin', 'process', 'processId', 'rate']):
                    ...
                    if not 'bin' in self.content['block1']:
                        if len(set(self.content['block2']['bin'])) > 1:
                            sys.exit(
                                'More than one bintype requires specific bin line together with observation line. Please add it.')
                        self.content['block1']['bin'] = list(
                            set(self.content['block2']['bin']))
                        self.linenumbers[('block1', 'bin')] = str(
                            float(self.linenumbers[('block1', 'observation')])-0.01)
            # systs
            elif self.advance[4]:
                parsed = False
                for systtype in ['lnN', 'lnU', 'shape', 'gmN', 'param', 'rateParam']:
                    if fields[1] == systtype or fields[1][0:5] == systtype:
                        # append if new
                        if not fields[1] in self.content['systs']:
                            self.content['systs'][fields[1]] = {}
                        # fill
                        ...
                        break
                if not parsed:
                    sys.exit('No algorithm to process line: %s with key %s' %
                             (line, fields[1]))
            # other
...
##################################################
    def addShape(self, name, number, root, workspace, hist=''):
        ...
        self.content['header2'][n] = [
            'shapes', name, number, root, workspace, hist]
        self.linenumbers[('header2', n)] = self.linenumbers[(
            'header2', str(int(n)-1))] + 0.01

##################################################
    def remShape(self, label, root='ALL'):
        if not (not label == '' and not root == ''):
            sys.exit('Arguments don\'t suffice, check these: label: ' +
                     str(label)+', root: '+str(root))
        ...
        for systtype in self.content['systs']:
            if systtype in ['lnN', 'lnU', 'gmN', 'shape'] or systtype[0:5] == 'shape':
                for entry in self.content['systs'][systtype].values():
                    entry.append('-')
        # header1
        self.content['header1']['imax'] = str(
            len(set(self.content['block1']['bin'])))

#FUNCTION: delCol################################
    def remCol(self, **kwargs):
        '''Method remCol.'''
        sets = [['bin', 'process'], ['bin', 'processId'],
                ['bin'], ['process'], ['processId']]
        iset = -999
        ...
        if iset == 2:
            if not kwargs['bin'] in self.content['block2']['bin']:
                sys.exit(
                    'No such bin in card, can\'t remove. Check arguments: '+str(kwargs))
            ...
            if not kwargs['process'] in self.content['block2']['process']:
                sys.exit(
                    'No such process in card, can\'t remove. Check arguments: '+str(kwargs))
            allprocesses = dc(self.content['block2']['process'])
            for process in allprocesses:
                if process == kwargs['process']:
                    self.remCol(bin=self.content['block2']['bin'][self.content['block2']['process'].index(
                        process)], process=process)
        elif iset == 4:
            if not kwargs['processId'] in self.content['block2']['processId']:
                sys.exit(
                    'No such processId in card, can\'t remove. Check arguments: '+str(kwargs))
            allprocesses = dc(self.content['block2']['processId'])
            for processid in allprocesses:
                if processid == kwargs['processId']:
                    self.remCol(bin=self.content['block2']['bin'][self.content['block2']['processId'].index(
            ...
            if 'process' in kwargs:
                index2 = self.commonIndex(
                    part='block2', bin=kwargs[sets[iset][0]], process=kwargs[sets[iset][1]])
            elif 'processId' in kwargs:
                index2 = self.commonIndex(
                    part='block2', bin=kwargs[sets[iset][0]], processId=kwargs[sets[iset][1]])
            # block2
            ...
            for systtype in self.content['systs']:
                if systtype in ['lnN', 'lnU', 'gmN', 'shape'] or systtype[0:5] == 'shape':
                    for tag in self.content['systs'][systtype]:
                        del self.content['systs'][systtype][tag][index2 if not systtype ==
                                                                 'gmN' else index2+1]
                    remtags = []
                    for tag in self.content['systs'][systtype]:
                    ...
                    for systtype, tag in remtags:
                        print('removed syst for tag %s: ' %
                              tag, self.content['systs'][systtype][tag])
            ...
            for entry in allbins:
                if not entry in self.content['block2']['bin']:
                    del self.content['block1']['observation'][self.content['block1']['bin'].index(
                        entry)]
                    del self.content['block1']['bin'][self.content['block1']['bin'].index(
                        entry)]
                    self.content['header1']['imax'] = str(
                        int(self.content['header1']['imax'])-1)
...
#FUNCTION: setCol################################
    def setCol(self, **kwargs):
        ...
        if not 'content' in kwargs:
            sys.exit(
                'Can\'t create new FILLED column if you don\'t pass content. Add content=... .')
        # start
        ...
        if not (('bin' in kwargs) and ('process' in kwargs or 'processId' in kwargs)):
            if passedcontent['block1']['bin'] in self.content['block1']['bin']:
                sys.exit('Bin label %s already exists in dictionary. Rename with bin+process+processId.' %
                         passedcontent['block1']['bin'])
            self.content['block1']['bin'].append(
                str(passedcontent['block1']['bin']))
        else:
            self.content['block1']['bin'].append(str(kwargs['bin']))
        self.content['block1']['observation'].append(
            str(passedcontent['block1']['observation']))
        # block2
        if not (('bin' in kwargs) and ('process' in kwargs or 'processId' in kwargs)):
            if passedcontent['block2']['bin'] in self.content['block2']['bin']:
                indices = [i for i in range(len(self.content['block2']['bin']))
                           if self.content['block2']['bin'][i] == passedcontent['block2']['bin']]
                for i in indices:
                    if self.content['block2']['process'][i] == passedcontent['block2']['process']:
                        sys.exit('(bin,process):(%s,%s) already exists in dictionary. Rename with bin+process+processId.' %
                                 (passedcontent['block2']['bin'], passedcontent['block2']['process']))
            self.content['block2']['bin'].append(
                str(passedcontent['block2']['bin']))
            self.content['block2']['process'].append(
                str(passedcontent['block2']['process']))
            self.content['block2']['processId'].append(
                str(passedcontent['block2']['processId']))
        else:
            if (not 'processId' in kwargs) or (not 'process' in kwargs):
                sys.exit(
                    'Defining new names needs full set bin/process/processId. Check arguments: '+str(kwargs))
            self.content['block2']['bin'].append(str(kwargs['bin']))
            self.content['block2']['process'].append(str(kwargs['process']))
            self.content['block2']['processId'].append(
                str(kwargs['processId']))
        self.content['block2']['rate'].append(
            str(passedcontent['block2']['rate']))
        # systs
        lookatsysts = ['lnN', 'lnU', 'gmN', 'shape']
        for systtype in self.content['systs']:
            if not (systtype in lookatsysts or systtype[0:5] == 'shape'):
                continue
                ...
                if systtype in passedcontent['systs']:
                    if entry in passedcontent['systs'][systtype]:
                        self.content['systs'][systtype][entry].append(
                            passedcontent['systs'][systtype][entry])
                    else:
                        self.content['systs'][systtype][entry].append('-')
        ...
        for systtype in passedcontent['systs']:
            ...
            if not systtype in self.content['systs']:
                ...
                for entry in passedcontent['systs'][systtype]:
                    ...
                    self.content['systs'][systtype][entry] = [
                        '-']*(len(self.content['block2']['bin'])-1)+[passedcontent['systs'][systtype][entry]]
                    nfrom += 1
...
#FUNCTION: commonIndex################################
    def commonIndex(self, **kwargs):
        ...
        if not 'part' in kwargs:
            sys.exit(
                'Not enough arguments to determine index. Check these: '+str(kwargs))
        ...
        if kwargs['part'] == 'block1':
            for ientry, entry in enumerate(self.content['block1']['bin']):
                if entry == kwargs['bin']:
                    ind1 += [ientry]
            if len(ind1) > 1:
                sys.exit('No unique index found. Check indices: %s and arguments: ' % str(
                    indices)+str(kwargs))
            indices = ind1
        # block2
        elif kwargs['part'] == 'block2':
...
##################################################
    def colwidth(self, part, field, syst='', systfield=-1):
        ...
        if not part == 'systs':
            if not field == 0:
                total = [len(x) for x in [y[(field-1)]
                                          for y in self.content[part].values() if (len(y) > (field-1))]]
            else:
                total = [len(x) for x in self.content[part]]
        else:
            if not field == 0:
                total = [len(x) for x in [
                    y[(field-1)] for y in self.content[part][syst].values() if (len(y) > (field-1))]]
            else:
                if systfield == 0:
...
#FUNCTION: getIndex###############################
    def getIndex(self, **kwargs):
        ...
        sets = [['tag', 'syst', 'bin', 'process'], ['tag', 'syst', 'bin', 'processId'], [
            'tag', 'syst', 'col'], ['bin', 'process'], ['bin', 'processId']]
        iset = -1
        ...
        if (iset < 0) or (iset >= i2 and 'syst' in kwargs):
            sys.exit(
                'Not enough arguments to identify which value to set. Correct these: '+str(kwargs))
        # rate case
        if iset >= i2 and iset < len(sets):
            for ix1, x1 in enumerate(self.content['block2'][sets[iset][0]]):
                if not x1 == kwargs[sets[iset][0]]:
                    continue
                if not self.content['block2'][sets[iset][1]][ix1] == kwargs[sets[iset][1]]:
                    continue
                index = ix1
        ...
        # syst cases (many cols)
            ...
            for ix1, x1 in enumerate(self.content['block2'][sets[iset][2]]):
                if not x1 == kwargs[sets[iset][2]]:
                    continue
                if not self.content['block2'][sets[iset][3]][ix1] == kwargs[sets[iset][3]]:
                    continue
        ...
        # syst cases (two cols)
            ...
            sys.exit(
                'Something is wrong with the arguments. Check these: '+str(kwargs))
...
#FUNCTION: setRate################################
    def setRate(self, **kwargs):
        ...
        if not 'value' in kwargs:
            sys.exit('No value specified to set.')
        index, keyset = self.getIndex(**kwargs)
...
#FUNCTION: setSyst################################
    def setSyst(self, **kwargs):
        ...
        if not 'value' in kwargs:
            sys.exit('No value specified to set.')
        index, keyset = self.getIndex(**kwargs)
        self.content['systs'][kwargs[keyset[1]]
                              ][kwargs[keyset[0]]][index] = str(kwargs['value'])
    ...
    def remSystLine(self, **kwargs):
        ...
        if iset == 0:
            if not kwargs['systtype'] in self.content['systs']:
                sys.exit('Systtype: %s is not present in the dictionary.' %
                         kwargs['systtype'])
            if not kwargs['tag'] in self.content['systs'][kwargs['systtype']]:
                sys.exit('Tag: %s is not present in the dictionary.' %
                         kwargs['tag'])
...
##################################################
    def addSystLine(self, **kwargs):
        ...
        for systtype in self.content['systs']:
            for tag in self.content['systs'][systtype]:
                if kwargs['tag'] == tag:
                    sys.exit('Systematic line with tag %s already exists, in systematic section: %s' % (
                        tag, systtype))
        # check type
        ...
        if kwargs['systtype'] == 'param':
            if not kwargs['systtype'] in self.content['systs']:
                self.content['systs'][kwargs['systtype']] = {}
            self.content['systs'][kwargs['systtype']][kwargs['tag']] = ['-']*2
            if kwargs['systtype'] in [x for x, y in self.linenumbers]:
                self.linenumbers[(kwargs['systtype'], kwargs['tag'])] = str(max([float(self.linenumbers[(
                    x, y)]) for x, y in list(self.linenumbers.keys()) if x == kwargs['systtype']])+0.01)
            else:
                self.linenumbers[(kwargs['systtype'], kwargs['tag'])] = str(max(
                    [float(self.linenumbers[(x, y)]) for x, y in self.linenumbers if x == 'param'])+10)
        elif kwargs['systtype'] in ['lnN', 'lnU', 'gmN'] or kwargs['systtype'][0:5] == 'shape':
            if not kwargs['systtype'] in self.content['systs']:
                self.content['systs'][kwargs['systtype']] = {}
            linelen = len(self.content['block2']['process'])
            self.content['systs'][kwargs['systtype']][kwargs['tag']] = (
                ['-'] if kwargs['systtype'] == 'gmN' else [])+['-']*linelen
            if kwargs['systtype'] in [x for x, y in self.linenumbers]:
                self.linenumbers[(kwargs['systtype'], kwargs['tag'])] = str(max([float(
                    self.linenumbers[(x, y)]) for x, y in self.linenumbers if x == kwargs['systtype']])+0.01)
            else:
                self.linenumbers[(kwargs['systtype'], kwargs['tag'])] = str(max(
                    [float(self.linenumbers[(x, y)]) for x, y in self.linenumbers if x == 'param'])+10)
        else:
...
# destination
        if not cardNameOut == '':
            if os.path.exists(cardNameOut):
                sys.exit('File %s already exists. NOT overwriting.' %
                         cardNameOut)
        ...
        for part in ['header1', 'header2', 'block1', 'block2']:
            if not part == 'header2':
                for key in sorted(self.content[part], key=lambda x: float(self.linenumbers[(part, x)])):
                    ...
                    print(
                        form % (key if not key in self.substitutes else self.substitutes[key]), end=' ')
...
def demo0():
    ...
    print('read CAT1, signal, rate: \t',
          dc.getRate(bin='CAT1', process='signal'))
    print('read CAT1, 0, rate: \t', dc.getRate(bin='CAT1', processId='0'))
    print('read CAT1, background, rate: \t',
          dc.getRate(bin='CAT1', process='background'))
    print('set CAT1, background, rate to 1.111')
    dc.setRate(bin='CAT1', process='background', value='1.111')
    print('read CAT, background, rate: \t',
          dc.getRate(bin='CAT1', process='background'))
    print()
    ####################
    info('Example of reading and setting systematic (lnN):')
    print('read CAT1, signal, lnN, lumi:\t', dc.getSyst(
        bin='CAT1', process='signal', syst='lnN', tag='lumi'))
    print('read CAT3, signal, lnN, trigEff:\t', dc.getSyst(
        bin='CAT3', process='signal', syst='lnN', tag='trigEff'))
    print('read CAT3, 0, lnN, trigEff:\t', dc.getSyst(
        bin='CAT3', processId='0', syst='lnN', tag='trigEff'))
    print('set CAT3, 0, lnN, trigEff to 1.111')
    dc.setSyst(bin='CAT3', processId='0', syst='lnN',
               tag='trigEff', value='1.111')
    print('read CAT3, 0, lnN, trigEff:\t', dc.getSyst(
        bin='CAT3', processId='0', syst='lnN', tag='trigEff'))
    print()
    ####################
    info('Example of reading and setting systematic (gmN):')
    print('read CAT1, signal, gmN, blablabla: \t', dc.getSyst(
        bin='CAT1', process='signal', syst='gmN', tag='blablabla'))
    print()
    ####################
    info('Example of reading and setting systematic (param):')
    print('read nzjet_CAT1, column 0: \t', dc.getSyst(
        syst='param', tag='nzjet_CAT1', col=0))
    ...
    dc.setSyst(bin='CAT5', process='signal',
               syst='lnN', tag='trigEff', value='1.033')
    dc.setSyst(bin='CAT5', process='signal', syst='lnN', tag='lumi', value=dc.getSyst(
        bin='CAT1', process='signal', syst='lnN', tag='lumi'))
    print()
    ####################
    ...
    ####################
    info('Example getCol()')
    dc.setCol(content=dc.getCol(bin='CAT1', process='signal',
              processId='0'), bin='CAT17', process='signal', processId='0')
    dc.write()
...
def demo1():
    ...
    print('read sf_vh2j, ggH, CMS_8TeV_eff_l, shape', dc.getSyst(
        bin='sf_vh2j', process='ggH', syst='shapeN2', tag='CMS_8TeV_eff_l'))
    print('read sf_vh2j, ggH, Gen_pow_WW, shape', dc.getSyst(
        bin='sf_vh2j', process='WW', syst='shapeN2', tag='Gen_pow_WW'))
    print('read sf_vh2j, ggH, Gen_pow_WW, shape', dc.getSyst(
        bin='sf_vh2j', process='VV', syst='shapeN2', tag='Gen_pow_WW'))
    print()
    ...
    dc.setCol(content=dc2.getCol(bin='j0sf7tev', process='ZH'),
              bin='addedbin', process='addedprocess', processId='99')
...
def myparser():
    mp = OptionParser()
    mp.add_option('--help_card', help='print info on how to use card class.',
                  action='store_true', default=False, dest='help_card')
    return mp

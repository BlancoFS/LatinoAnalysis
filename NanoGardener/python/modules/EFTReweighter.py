...
class EFTReweighter(Module):
    def __init__(self, sample):
        ...
        ROOT.gSystem.Load(
            self.cmssw_base+"/src/JHUGenMELA/MELA/data/"+self.cmssw_arch+"/libmcfm_707.so")

        try:
            ROOT.gROOT.LoadMacro(
                self.cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/melaHiggsEFT.C+g')
        except RuntimeError:
            ROOT.gROOT.LoadMacro(
                self.cmssw_base+'/src/LatinoAnalysis/Gardener/python/variables/melaHiggsEFT.C++g')
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        print(("Running MELA EFT reweighter with " +
              self.productionProcess + " sample"))

        self.out = wrappedOutputTree

        if self.vertexType == "HVV":
            self.newbranches = ['gen_dme_hsm', 'gen_dme_hm', 'gen_dme_hp',
                                'gen_dme_hl', 'gen_dme_mixhm', 'gen_dme_mixhp', 'gen_dme_mixhl']
            if self.productionProcess != "GluGlu":
                self.newbranches += ['gen_pme_hsm', 'gen_pme_hm', 'gen_pme_hp',
                                     'gen_pme_hl', 'gen_pme_mixhm', 'gen_pme_mixhp', 'gen_pme_mixhl']
    ...
    #      print "genp ", gen.pdgId, event.GenPart_pdgId[mid], self.FromH(event, gid)
            if abs(event.GenPart_pdgId[mid]) != 24:
                continue
            if self.FromH(event, gid) == False:
                continue
        ...
        if len(LHEHFinalState) != 4:
            print("SOMETHING WENT WRONG!, WW final state",
                  len(LHEHFinalState), LHEHFinalState)

        for ipart in LHEHFinalState:
            d = ROOT.TLorentzVector()
            d.SetPtEtaPhiM(
                LHEHFinalState[ipart][0], LHEHFinalState[ipart][1], LHEHFinalState[ipart][2], 0.)
        ...
        if self.sample is "H0L1f05_ToWWTo2L2Nu":
            if abs(daughterIDs[0]) in [11, 13, 15] and abs(daughterIDs[1]) in [12, 14, 16] and abs(daughterIDs[2]) in [12, 14, 16] and abs(daughterIDs[3]) in [11, 13, 15]:
                if self.MatchLNuIDs(daughterIDs[0], daughterIDs[1]) is False:
                    ...
                    daughterIDs[1] = -1 * \
                        numpy.sign(daughterIDs[0])*(abs(daughterIDs[0])+1)
                if self.MatchLNuIDs(daughterIDs[3], daughterIDs[2]) is False:
                    print("2nd Lep-Nu pair IDs not matching : ",
                          daughterIDs[3], daughterIDs[2])
                    daughterIDs[2] = -1 * \
                        numpy.sign(daughterIDs[3])*(abs(daughterIDs[3])+1)

        mothers = ROOT.vector('TLorentzVector')()
        motherIDs = ROOT.vector('int')()
        incoming1 = ROOT.TLorentzVector()
        incoming1.SetPxPyPzE(0., 0., event.Generator_x1 *
                             6500, event.Generator_x1*6500)
        incoming2 = ROOT.TLorentzVector()
        incoming2.SetPxPyPzE(0., 0., -1*event.Generator_x2 *
                             6500, event.Generator_x2*6500)
        ...
        if self.productionProcess == "ZH" or self.productionProcess == "WH":
            ...
            if len(LHEVFinalState) != 2:
                print("SOMETHING WENT WRONG!, V final state ",
                      len(LHEVFinalState), VFinalStateIdx)

            for ipart in LHEVFinalState:
                add = ROOT.TLorentzVector()
                add.SetPtEtaPhiM(
                    LHEVFinalState[ipart][0], LHEVFinalState[ipart][1], LHEVFinalState[ipart][2], 0.)
                adds.push_back(add)
            ...
            for ijet in LHEjetIdx:
                add = ROOT.TLorentzVector()
                add.SetPtEtaPhiM(
                    event.LHEPart_pt[ijet], event.LHEPart_eta[ijet], event.LHEPart_phi[ijet], 0.)
                adds.push_back(add)
        ...
        for idx, dau in enumerate(daughters):
            daughter_coll.push_back(
                ROOT.SimpleParticle_t(daughterIDs[idx], dau))
        ...
        if self.vertexType == "HVV":
            ...
            if self.productionProcess == "GluGlu":
                DME = ROOT.melaHiggsEFT(
                    self.mela, ROOT.TVar.JHUGen, ROOT.TVar.ZZINDEPENDENT, 1, 0)
            else:
                DME = ROOT.melaHiggsEFT(
                    self.mela, ROOT.TVar.JHUGen, ROOT.TVar.ZZINDEPENDENT, 0, 0)
                PME = ROOT.melaHiggsEFT(
                    self.mela, ROOT.TVar.JHUGen, self.productionMela, 0, 0)
            ...
            PME = ROOT.melaHiggsEFT(
                self.mela, ROOT.TVar.JHUGen, self.productionMela, 1, 0)
        ...
        if math.isnan(gen_pme_hsm):
            print("SOMETHING WENT WRONG?, Production ME is nan ")
        if math.isnan(gen_dme_hsm):
            print("SOMETHING WENT WRONG?, Decay ME is nan ")
    ...
    def pTorder(self, event, oldlist):
        ...
        for i, pone in enumerate(oldlist):
            for j, ptwo in enumerate(oldlist):
                if pone == ptwo:
                    continue
                if event.LHEPart_pt[pone] > event.LHEPart_pt[ptwo]:
                    order[i] += 1
    ...
    def MatchLNuIDs(self, idL, idNu):
        if idNu == -1*numpy.sign(idL)*(abs(idL)+1):
            return True
        else:
            return False
    ...
    def RemoveGammaW(self, event, FinalStateIdx):  # Remove W -> gamma W -> e+ e- W
        ...
        for pi1, p1 in enumerate(FinalStateIdx):
            for pi2, p2 in enumerate(FinalStateIdx):
                if pi1 >= pi2:
                    continue
                if event.GenPart_genPartIdxMother[p1] != event.GenPart_genPartIdxMother[p2]:
                    continue
                if event.GenPart_pdgId[p1] + event.GenPart_pdgId[p2] == 0:
                    if p1 not in removethis:
                        removethis.append(p1)
                    if p2 not in removethis:
    ...
    def RemoveAddHadron(self, event, FinalStateIdx):  # Remove rare additonal hadrons
        ...
        for pi1, p1 in enumerate(FinalStateIdx):
            ...
            NumWithThisID = 0
            for pi2, p2 in enumerate(FinalStateIdx):
                if event.GenPart_genPartIdxMother[p2] == mom_id:
                    NumWithThisID += 1
            if NumWithThisID != 2:
                for pi2, p2 in enumerate(FinalStateIdx):
    ...
    # Particles in LHE collection have higher precision -> Find LHE particles with closest match to GenParticles
    def getLHE(self, event, genlist):
        LHElist = {}
        for gid in genlist:

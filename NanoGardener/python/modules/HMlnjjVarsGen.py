    ...
    def analyze(self, event):
        ...
        if self.DataMc == 'MC':
            ...
            if singleLeptEvt:
                ...
                gSingleLept_pz = gSingleLept_pt * \
                    math.sinh(gSingleLept_eta)  # pz = pt*sinh(eta)
                gSingleLept_E = gSingleLept_pt * \
                    math.cosh(gSingleLept_eta)  # p = pt*cosh(eta)
                self.gSingleLept_v4.SetPxPyPzE(
                    gSingleLept_px, gSingleLept_py, gSingleLept_pz, gSingleLept_E)
                ...
                for igAk8 in range(GenAK8_col._len):
                    ...
                    if gW_Ak8_pt < 200:
                        continue
                    if abs(gW_Ak8_eta) > 2.4:
                        continue
                    if gW_Ak8_mass > 105:
                        continue
                    if gW_Ak8_mass < 65:
                        continue
                    dRAk8Lept = self.getDeltaR(
                        gW_Ak8_phi,  gW_Ak8_eta, gSingleLept_phi, gSingleLept_eta)
                    if dRAk8Lept < 0.8:
                        continue
                    ...
                    for igAk4 in range(GenAK4_col._len):
                        ...
                        if abs(gAk4_eta) > 2.4:
                            continue
                        dRAk8Ak4 = self.getDeltaR(
                            gW_Ak8_phi,  gW_Ak8_eta, gAk4_phi, gAk4_eta)
                ...
                if not genSemiLeptFatJetEvt:
                    ...
                    for idx in range(GenAK4_col._len):
                        ...
                        if idx == gAk4_idx0 or idx == gAk4_idx1:
                            continue
                        if gAk4_0_pt < 20:
                            continue
                        if abs(gAk4_0_eta) > 2.4:
                    ...
                    if genSemiLeptResolvEvt:
                        ...
                        gW_Ak4_v4_0.SetPtEtaPhiM(
                            gResJet_0_pt, gResJet_0_eta, gResJet_0_phi, gResJet_0_mass)
                        ...
                        gW_Ak4_v4_1.SetPtEtaPhiM(
                            gResJet_1_pt, gResJet_1_eta, gResJet_1_phi, gResJet_1_mass)
                        self.gW_Ak4_v4 = gW_Ak4_v4_0 + gW_Ak4_v4_1
                        dRAk4Lept[0] = self.getDeltaR(
                            gResJet_0_phi,  gResJet_0_eta, gSingleLept_phi, gSingleLept_eta)
                        dRAk4Lept[1] = self.getDeltaR(
                            gResJet_0_phi,  gResJet_0_eta, gSingleLept_phi, gSingleLept_eta)
            ...
            if genSemiLeptFatJetEvt:
                ...
                self.gW_Ak8_v4.SetPtEtaPhiM(
                    gW_Ak8_pt, gW_Ak8_eta, gW_Ak8_phi, gW_Ak8_mass)
...
def HMlnjjVarsGen(): return HMlnjjVarsGenClass()

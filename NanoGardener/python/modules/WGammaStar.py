...
class WGammaStarV2(Module):
    ...
    def findGStarPair(self, leptons):
        ...
        charged = [l for l in leptons if abs(l.pdgId) == 11 or abs(
            l.pdgId) == 13 or abs(l.pdgId) == 15]
        pairs = []
...
5= from a photon in the parton shower")
    ...
    def analyze(self, event):
        ...
        gstar = None
        statuses = ['fromG_HP', 'fromZ_ll_inhistory',
                    "fromZ_llll", 'fromZ_ll_decay', 'fromW3l', 'fromG_PS']
        ...
        if gstar != None:
            ...
            if abs(self.pLeptState1[0].pdgId) == 13:
                ...
                self.lept1_4V.SetPtEtaPhiM(
                    muon1FromGstar_pt, muon1FromGstar_eta, muon1FromGstar_phi, 0.106)
                self.lept2_4V.SetPtEtaPhiM(
                    muon2FromGstar_pt, muon2FromGstar_eta, muon2FromGstar_phi, 0.106)
            else:
                ...
                self.lept1_4V.SetPtEtaPhiM(
                    elec1FromGstar_pt, elec1FromGstar_eta, elec1FromGstar_phi, 0.0005)
                self.lept2_4V.SetPtEtaPhiM(
                    elec2FromGstar_pt, elec2FromGstar_eta, elec2FromGstar_phi, 0.0005)
            _ZGstarDiLept_DelaR = self.deltaR(
                self.pLeptState1[0].eta, self.pLeptState1[1].eta, self.pLeptState1[0].phi, self.pLeptState1[1].phi)

...
class jetRecalib(Module):
    ...
    def __init__(self,  globalTag, jetCollections=["CleanJet"], metCollections=["MET"], jetType="AK4PFchs"):
        ...
        # Any jet collections based on full Jet-collection
        self.otherJetBranches = jetCollections
        self.metCollections = metCollections
        self.rhoBranchName = "fixedGridRhoFastjetAll"
        # To do : change to real values
        self.jmsVals = [1.00, 0.99, 1.01]

        self.jesInputFilePath = os.environ['CMSSW_BASE'] + \
            "/src/PhysicsTools/NanoAODTools/data/jme/"

        self.jetReCalibrator = JetReCalibrator(
            globalTag, jetType, True, self.jesInputFilePath, calculateSeparateCorrections=False, calculateType1METCorrection=False)
    ...
    def analyze(self, event):
        ...
        for jid, jet in enumerate(jets):
            ...
            rawFactor_newlist.append(
                1. - ((jet.pt * (1. - jet.rawFactor))/newjet_pt))
        ...
        self.out.fillBranch("%s_rawFactor" %
                            self.jetBranchName, rawFactor_newlist)
        for oj, jname in enumerate(self.otherJetBranches):
            self.out.fillBranch("%s_pt" % jname, otherjets_pt_newlist[oj])
        for mid, met in enumerate(self.metCollections):
            self.out.fillBranch("%s_pt" % met, math.sqrt(
                met_px[mid]**2 + met_py[mid]**2))
            self.out.fillBranch("%s_phi" %
                                met, math.atan2(met_py[mid], met_px[mid]))
...
def jetRecalib2017B(): return jetRecalib("Fall17_17Nov2017B_V32_DATA",
                                         jetCollections=["CleanJet"], metCollections=["MET"])


def jetRecalib2017C(): return jetRecalib("Fall17_17Nov2017C_V32_DATA",
                                         jetCollections=["CleanJet"], metCollections=["MET"])


def jetRecalib2017D(): return jetRecalib("Fall17_17Nov2017DE_V32_DATA",
                                         jetCollections=["CleanJet"], metCollections=["MET"])


def jetRecalib2017E(): return jetRecalib("Fall17_17Nov2017DE_V32_DATA",
                                         jetCollections=["CleanJet"], metCollections=["MET"])


def jetRecalib2017F(): return jetRecalib("Fall17_17Nov2017F_V32_DATA",
                                         jetCollections=["CleanJet"], metCollections=["MET"])


def jetRecalib2018A(): return jetRecalib("Autumn18_RunA_V19_DATA", jetCollections=[
    "CleanJet"], metCollections=["PuppiMET"], jetType="AK4PFchs")


def jetRecalib2018B(): return jetRecalib("Autumn18_RunB_V19_DATA", jetCollections=[
    "CleanJet"], metCollections=["PuppiMET"], jetType="AK4PFchs")


def jetRecalib2018C(): return jetRecalib("Autumn18_RunC_V19_DATA", jetCollections=[
    "CleanJet"], metCollections=["PuppiMET"], jetType="AK4PFchs")


def jetRecalib2018D(): return jetRecalib("Autumn18_RunD_V19_DATA", jetCollections=[
    "CleanJet"], metCollections=["PuppiMET"], jetType="AK4PFchs")
